...
// index메서드 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    public function index(Request $request){ // 메인페이지, Request 클래스의 변수를 배개변수로 사용(컨트롤러로 넘어오는 Request값을 받기 위해 필요)
        ...
        $comments = Task::orderby('grp', 'DESC') // 테이블에서 가져온 DB 뷰에서 사용 할 수 있는 변수에 저장.
        ->orderby('sort')->skip($startNum)->take($writeList)->get();

        return view('tasks.index', [ // 요청된 정보 처리 후 결과 되돌려줌, 경로를 작성할 때 \대신 .을 사용 = tasks폴더 안의 index.blade.php라는 의미
