<x-app-layout>
    <div>
        {{-- <figure id="section" class="flex flex-col items-center w-screen h-screen mt-16 md:max-w-2xl"> --}}
            <div class="flex mb-4">
                    <img class="w-2/4 h-50" src="/storage/images/gosung1669496902.png">
                <div class="h-1 ml-8 w-4/4">
                    <p class="mt-16 text-4xl tracking-widest">딸기인형</p>
                    <p class="mt-8 text-xl tracking-widest text-gray-500">맞벌이 가정이 많아지고 있는 사회 속에서</p>
                    <p class="mt-4 text-xl tracking-widest text-gray-500">아이가 유일하게 항상 믿고 의지할 수 있는 친구.</p>
                </div>
            </div>
        {{-- </figure> --}}
        <figure id="section" class="flex flex-col items-center justify-center w-screen h-screen">
            <img class="w-3/4 h-80"
                src="https://mblogthumb-phinf.pstatic.net/MjAyMDA0MDlfOTEg/MDAxNTg2NDMxMzk3Mzk1.-Mqd0Lonz1iJV6oOn3A1P4Rb2diLqQTrldaNQP29tswg.K1apwu2919Cnk2z46HUxu2nodpCkuDz9VVNhpLqzXqQg.PNG.arum815/%EB%94%B8%EA%B8%B03.png?type=w800">
            <p class="mt-16 text-5xl tracking-widest text-red-500">아름다운 여러분들</p>
            <p class="mt-8 text-xl tracking-widest text-gray-800">우리의 머리털은 지키고</p>
            <p class="mt-4 text-xl tracking-widest text-gray-500">맘뉼이의 탈모를 기원해봐요</p>
        </figure>
        <figure id="section" class="flex flex-col items-center justify-center w-screen h-screen">
            <img class="w-3/4 h-80"
                src="https://mblogthumb-phinf.pstatic.net/MjAyMDA0MDlfMjE5/MDAxNTg2NDMxMzk0NDI0.YYb-SnZsp1tX_Agjaxvhscn7XZnem10y5BJgNY0oKpMg.4J6Sk5WeUco1F82Pdb9CuYDoyGaf_38t2ApnUzEJEGUg.PNG.arum815/%EB%94%B8%EA%B8%B02.png?type=w800">
            <p class="mt-16 text-4xl tracking-widest">우리들은 시크동현인형을 팝니다</p>
            <p class="mt-8 text-xl tracking-widest text-gray-500">성별은 여자예요</p>
            <p class="mt-4 text-xl tracking-widest text-gray-500">커스터마이징이 가능하답니다</p>
            <p class="mt-4 text-xl tracking-widest text-red-300">이름도 마음대로 바꿀 수 있어요!</p>
        </figure>
        <figure id="section" class="flex flex-col items-center justify-center w-screen h-screen">
            <img class="w-3/4 h-80" src="https://t1.daumcdn.net/cfile/tistory/99D2E53C5BD1EB3F18">
            <p class="mt-16 text-4xl tracking-widest">여러분 저 어도비 결제 못해서</p>
            <p class="mt-8 text-xl tracking-widest text-gray-500">작품을 못 만들어요</p>
            <p class="mt-4 text-xl tracking-widest text-gray-500">2만원만 주세요</p>
            <p class="mt-4 text-xl tracking-widest text-gray-500">응애~~~~~</p>
        </figure>
    </div>
</x-app-layout>